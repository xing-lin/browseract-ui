...
- 双色 Icon 目前是黑色 + 绿色，使用 `:root` 暴露的变量
  - `data-hover: "black" | "green"` 用于元素（父容器） hover 时改变颜色
  - `data-render: "stroke" | "fill"` 用于元素（父容器） hover 时使用正确填充类型
  - Demo:

    ```css
    .icon-btn-hover:hover [data-hover='green']:not([data-render='stroke']) {
